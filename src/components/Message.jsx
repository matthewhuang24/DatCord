...
const Message = props => {
  ...
  return (
    <form className='messageForm'>
      <input id={props.id} value={updatedMessage}  size="50" onChange={() => {onUpdatedMessageChange(event, props.id)}} className="messages" disabled/>
      <span>
        <button value={updatedMessage} onClick={()=>onEdit(props.id, event)}>Edit</button>
        <button onClick={()=>{props.deleteMessage(props.id, event)}}>X</button>
      </span>
    </form>
  )
};
